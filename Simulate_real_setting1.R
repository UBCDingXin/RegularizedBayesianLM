...
#3.2 setting 1
# setups
Setting_name = "_setting1_"
prior = "horseshoe" #"ridge" or "horseshoe"
...
for (nrho in 1:length(rho2_grid)){
  for (nn in 1:nSim){
    ...
    # fit_tmp = bayeslm(Z ~ X, prior=prior, icept=FALSE, N=nsamps, verb=FALSE, sigma = sigma2_ep)
    # Zw = Z - X %*% fit_tmp$beta[2:31]
    # fit1=bayeslm(Y~cbind(Zw,X), prior=prior, penalize=c(0,rep(1,p)),
    #              icept=FALSE, N=nsamps, verb=FALSE, sigma = sigma2_nu)
    # fit1_coeff[nn,1,nrho]=mean(as.matrix(fit1$beta)[,2])#why setting icept=FALSE is useless?
    # fit1_coeff[nn,2,nrho]=sd(as.matrix(fit1$beta)[,2])
    # bias1[nn,nrho]=fit1_coeff[nn,1,nrho]-alpha
    # CI1[nn,1,nrho]=fit1_coeff[nn,1,nrho]-fit1_coeff[nn,2,nrho]*cutoff_point
    # CI1[nn,2,nrho]=fit1_coeff[nn,1,nrho]+fit1_coeff[nn,2,nrho]*cutoff_point
    # IL1[nn,nrho] = 2*fit1_coeff[nn,2,nrho]*cutoff_point
    ...
    IL2[nn,nrho] = 2*fit2_coeff[nn,2,nrho]*cutoff_point
