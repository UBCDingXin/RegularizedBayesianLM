...
nsamps = 5000
finalStep = 2 # 2 means with final step, sample from normal
nSim=1000#number of simulations
n=1000#sample size
p=52#the number of all regressors
rho=0.7#cov of Z and X1
Sigma_ZXX=diag(3)#cov of Z, X1, and X2
Sigma_ZXX[1,2]=Sigma_ZXX[2,1]=rho
Sigma_all=diag(p)#cov of Z and all Xs
Sigma_all[1:3,1:3]=Sigma_ZXX

alpha=beta1=beta2=0.1#coefficients
...
for (nn in 1:nSim){
  ...
  Y=alpha*Z+beta1*X1+beta2*X2+err
  #-------------------------------------------
  #Fit models
  #new approach:fit1
  if (prior=="horseshoe"){
  ...
  IL1[nn] = 2*fit1_coeff[nn,2]*qnorm(0.975,0,1)
  ...
  IL2[nn] = 2*fit2_coeff[nn,2]*qnorm(0.975,0,1)
  ...
  IL3[nn] = 2*fit3_coeff[nn,2]*qnorm(0.975,0,1)
  ...
  IL4[nn] = 2*fit4_coeff[nn,2]*qnorm(0.975,0,1)
  ...
  IL5[nn] = 2*fit5_coeff[nn,2]*qnorm(0.975,0,1)
}
#avg bias
...
# save.image("Wang_sim1_horse.Rdata")


file = paste0("Wang_sim1_",prior,".Rdata",seq="")
save.image(file)
